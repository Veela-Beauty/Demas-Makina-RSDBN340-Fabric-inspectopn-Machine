...
class App:
    def __init__(self):
        ...
        i18n.I18n.set_lang(self.settings.get("language", "en"))
    ...
    def _try_auto_login(self):
        ...
        usr = self.settings.get("username", "").strip()
        pwd = self.settings.get("password", "")
        if not (base and usr and pwd):
            return
        threading.Thread(target=self._do_auto_login, args=(base, usr, pwd), daemon=True).start()

    def _do_auto_login(self, base, usr, pwd):
        try:
            client = ErpnextClient(base, verify_tls=self.settings.get("verify_tls", True))
            info = client.login(usr, pwd)
        except Exception:
            return
        self.settings["base_url"] = base
        self.settings["username"] = usr
        self.settings["password"] = pwd
        self.after(0, lambda: self._on_login(client, info.get("full_name") or usr))
    ...
    def _toggle_lang(self):
        new = "ar" if i18n.I18n._lang == "en" else "en"
        i18n.I18n.set_lang(new)
    ...
    def _build_top_bar(self):
        ...
        theme.label(bar, i18n._("top.port"), muted=True).pack(side=S("left"), padx=P((16, 6)))
        ...
        self.port_combo = ctk.CTkComboBox(bar, values=ports, variable=self.port_var, width=110,
                                          ...
                                          font=theme.font(13))
        self.port_combo.pack(side=S("left"), padx=4)
        theme.ghost_button(bar, i18n._("top.refresh"), self._refresh_ports, width=84).pack(side=S("left"), padx=6)
        self.connect_btn = theme.success_button(bar, i18n._("top.connect"), self._on_connect_toggle, width=110)
        self.connect_btn.pack(side=S("left"), padx=6)
        self.led = ctk.CTkFrame(bar, width=12, height=12, corner_radius=6, fg_color=theme.DANGER)
        self.led.pack(side=S("left"), padx=P((10, 6)))
        self.status_var = tk.StringVar(value=i18n._("top.disconnected"))
        ctk.CTkLabel(bar, textvariable=self.status_var, font=theme.font(13),
                     text_color=theme.MUTED).pack(side=S("left"))

        theme.ghost_button(bar, "?", self._show_about, width=34).pack(side=S("right"), padx=P((6, 16)))
        mode = "Dark" if ctk.get_appearance_mode() == "Light" else "Light"
        lang_label = i18n.I18n.lang_name()
        theme.ghost_button(bar, lang_label, self._toggle_lang, width=70).pack(side=S("right"), padx=6)
        theme.ghost_button(bar, mode, self._toggle_theme, width=70).pack(side=S("right"), padx=6)
        theme.ghost_button(bar, i18n._("top.switch_user"), self._switch_user, width=110).pack(side=S("right"), padx=6)

        chip = ctk.CTkFrame(bar, fg_color="transparent")
        chip.pack(side=S("right"), padx=6)
        ...
        ctk.CTkLabel(chip, text=initials, width=28, height=28, corner_radius=14,
                     ...
                     font=theme.font(11, "bold")).pack(side=S("left"), padx=P((0, 8)))
        theme.label(chip, name, weight="bold").pack(side=S("left"))
    ...
    def _on_connect_toggle(self):
        if self._connected:
            ...
            if not self._last_port or self._last_port == "(none)":
                self._toast(i18n._("top.select_port"))
                return
            self.worker.submit("connect", self._last_port)

    def _set_connected(self, port):
        self.connect_btn.configure(text=i18n._("top.disconnect"), fg_color=theme.DANGER, hover_color=theme.DANGER)
        self.led.configure(fg_color=theme.SUCCESS)
        self.status_var.set(i18n._("top.connected").format(port))

    def _set_disconnected(self):
        self.connect_btn.configure(text=i18n._("top.connect"), fg_color=theme.SUCCESS, hover_color=theme.SUCCESS_HI)
        self.led.configure(fg_color=theme.DANGER)
        self.status_var.set(i18n._("top.disconnected"))
    ...
    def _handle_result(self, cmd, result):
        if cmd == "read_meters":
            ...
            self._toast(i18n._("cfg.preset_sent") if result else i18n._("cfg.preset_fail"))
        elif cmd == "write_error_detail":
            self._toast(i18n._("cfg.detail_sent") if result else i18n._("cfg.detail_fail"))
        elif cmd == "connect":
            if result:
                self._connected = True
                self._set_connected(self._last_port)
            else:
                self._toast(i18n._("top.connect_fail").format(self._last_port))
        elif cmd == "disconnect":
            self._connected = False
